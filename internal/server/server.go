...
import (
	...
	"github.com/jbakhtin/driving-school-route-coverage/internal/server/handlers/pingsql"
...
func (s *Server) Start(ctx context.Context) error {
	...
	// handlers
	authHandler, err := auth.NewHandler(*s.config)
	if err != nil {
		return err
	}

	areaHandler, err := area.NewHandler(*s.config)
	if err != nil {
		return err
	}

	markHandler, err := mark.NewHandler(*s.config)
	if err != nil {
		return err
	}

	routeHandler, err := route.NewHandler(*s.config)
	if err != nil {
		return err
	}

	pingDBHandler, err := pingsql.NewHandler(*s.config)
	if err != nil {
		return err
	}
	...
	r.Route("/", func(r chi.Router) {
		...
		r.Route("/areas", func(r chi.Router) {
			...
			r.Get("/points", areaHandler.GetPointsGroupByAreas()) // Получить все точки сгруппированные по областям
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Get("/points", areaHandler.GetPoints())
			})
		})
		...
		r.Route("/routes", func(r chi.Router) {
			...
			r.Get("/points", routeHandler.GetPointsGroupByRoutes()) // Получить точки маршрутов сгруппированных по маршрутам
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Get("/points", routeHandler.GetPoints()) // Получить точки по конкретному маршруту
			})
		})
		...
		r.Route("/marks", func(r chi.Router) {
			...
			r.Get("/points", markHandler.GetPoints()) // Получить все точки сгруппированные по маркам
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Get("/point", markHandler.GetPoint()) // Получить точку по конкретной марке
			})
		})


		r.Route("/ping/", func(r chi.Router) {
			r.Get("/sql", pingDBHandler.PingSQL())
		})
	})
